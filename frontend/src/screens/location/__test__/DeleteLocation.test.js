...
import Delete from '../DeleteLocation'; // Adjust the path as needed
...
describe('Delete', () => {
    ...
    test('renders the dialog with correct location information', () => {
        const selectedRow = {
    ...
    test('calls successToast and fetchRow on successful delete', async () => {
        ...
        await waitFor(() => {
            expect(successToast).toHaveBeenCalledWith("Location deleted successfully!");
    ...
    test('shows error toast when delete returns 404 status', async () => {
        apiClient.delete.mockResolvedValueOnce({
            data: { success: false, status: 404, message: "Location not found." },
        });
    ...
    test('shows error toast when delete fails', async () => {
        ...
        await waitFor(() => {
            expect(errorToast).toHaveBeenCalledWith("Failed to delete location");
            expect(mockSetOpenDeleteDialog).toHaveBeenCalledWith(false);
        });
    });
