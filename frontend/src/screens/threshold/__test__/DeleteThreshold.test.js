...
import Delete from '../DeleteThreshold'; // Adjust the path as needed
...
describe('Delete', () => {
    ...
    test('renders the dialog with correct information', () => {
        ...
        expect(screen.getByText(/Are you sure you want to delete this threshold?/i)).toBeInTheDocument();
    });
    ...
    test('calls successToast and fetchRow on successful delete', async () => {
        ...
        await waitFor(() => {
            expect(successToast).toHaveBeenCalledWith("Threshold deleted successfully!");
    ...
    test('shows error toast when delete returns 404 status', async () => {
        apiClient.delete.mockResolvedValueOnce({
            data: { success: false, status: 404, message: "Threshold not found." },
        });
        ...
        await waitFor(() => {
            expect(errorToast).toHaveBeenCalledWith("Threshold not found.");
            expect(mockSetOpenDeleteDialog).toHaveBeenCalledWith(false);
        });
    });
    ...
    test('shows error toast when delete fails', async () => {
        ...
        await waitFor(() => {
            expect(errorToast).toHaveBeenCalledWith("Failed to delete Threshold");
            expect(mockSetOpenDeleteDialog).toHaveBeenCalledWith(false);
        });
    });
